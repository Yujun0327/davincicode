...
export function playerKey(): string {
  const KEY = 'davincicode:player-key'
...
export function loadPlayerName(): string {
  return localStorage.getItem('davincicode:player-name') ?? ''
}

export function savePlayerName(name: string): void {
  localStorage.setItem('davincicode:player-name', name)
}
...
const roomKey = (room: string, key: string) => `davincicode:room:${room.toUpperCase()}:${key}`
