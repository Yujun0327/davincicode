...
/**
 * davincicode on the shared beacon session (see @yujun/game-net). Turn-holder
  ...
  return {
    ...
    makeConfig: (players, prev) => {
      ...
      return {
        ...
        rulesVersion: RULES_VERSION,
      jokers: prev ? prev.jokers : (host()?.jokersWanted ?? true),
      }
    },
    ...
    winners: (s) => (s.result ? [s.result.winner] : []),
  }
}
