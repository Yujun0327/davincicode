<script lang="ts">
  ...
  function syncFromHash() {
    ...
    if (room && online?.room !== room) {
      online?.destroy()
      const creator = sessionStorage.getItem(`davincicode:creator:${room}`) !== null
  ...
  $effect(() => {
    ;(window as unknown as Record<string, unknown>).__davincicode = online ?? hotseat
  })
  ...
  function createRoom() {
    ...
    sessionStorage.setItem(`davincicode:creator:${code}`, '1')
